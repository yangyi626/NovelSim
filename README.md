...
- 质量：446 个本地确定性测试、9 案例结构化场景评测、20 局真实 LLM
...
2026-08-15完成 Web 规划闭环：使用 `.env` 中的真实 LLM 生成 `draft`，允许人工
修改结构化动作链，经 `approved` 后才能进入受限工具执行；支持单步、整轮以及
1--10 轮有界 Auto 推演。隔离真实模型烟测已完成 `draft -> approved -> completed`
和 `WorldEvent` 提交，世界版本由 `v0` 推进至 `v1`。
