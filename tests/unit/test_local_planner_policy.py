...
def test_checkpoint_inspection_requires_manifest_and_exact_file_hashes(tmp_path):
    ...
    manifest_path.write_text(json.dumps({
        ...
        "validation": {
            "prompt_version": "novelsim_planner_prompt.v2",
            "dataset_id": "novelsim_planner_sft_v2",
        },
        "code_commit": "abc1234",
    ...
    assert ready.dataset_id == "novelsim_planner_sft_v2"
...
def test_grpo_checkpoint_requires_grpo_manifest_and_parent_sft_hashes(tmp_path):
    ...
    manifest_path.write_text(json.dumps({
        ...
        "validation": {
            "prompt_version": "novelsim_planner_prompt.v2",
            "dataset_id": "novelsim_planner_grpo_v2",
        },
    ...
    ready = inspect_adapter_checkpoint(config, repo_root=tmp_path)
    assert ready.ready is True
    assert ready.dataset_id == "novelsim_planner_grpo_v2"
