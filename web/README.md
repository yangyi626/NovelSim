...
- **右栏·世界检查器**：在状态、规则判定、场景和协作计划标签页之间切换；非法行动会自动打开规则判定并显示拒绝码、原因和未提交状态提示。
- **规划审批台**：使用 `.env` 中的真实 LLM 生成角色私有的结构化动作链；Manual 模式可编辑 JSON、保存、批准后单步或整轮执行，Auto 模式按设置轮数自动生成、批准、执行并在阻塞时停止。
...
- `POST /api/joint-plans/generate` 生成不可执行的 `draft`；`PUT /api/joint-plans/<id>` 修改并重新校验；`POST .../approve` 冻结当前世界版本；`POST .../execute` 通过受限工具单步或整轮执行。
